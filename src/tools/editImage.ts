...
import { Stream } from 'stream'; // For download stream typing
...
export const editImageSchema = editImageBaseSchema.refine(data => !!data.image_url !== !!data.image_path, { // Ensure exactly one image source is provided
...
// Reuse Gemini response structure (might be the same for editing)
...
async function downloadImageToTemp(url: string, tempDir: string): Promise<{ tempPath: string, mimeType: string | null }> {
    ...
    try {
        ...
        const stream = response.data as Stream; // Type assertion for stream
        stream.pipe(writer);

        await new Promise<void>((resolve, reject) => { // Add void type for Promise
...
async function getImageDataFromPath(filePath: string): Promise<{ base64Data: string, mimeType: string }> {
    try {
...
export async function handleEditImage(
    ...
    try {
        ...
        const editedImagePart = editedParts?.find((part: GeminiContentPart) => part.inlineData && part.inlineData.mimeType.startsWith('image/'));
