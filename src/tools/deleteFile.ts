...
export const deleteFileSchema = z.object({
    fileName: z.string().min(1).regex(/^files\/[a-zA-Z0-9]+$/, "fileName must be in the format 'files/xxxxxx'")
        .describe("Required. The name of the file to delete (e.g., 'files/kch7l0eddn96'). Get this from the listFiles tool."),
...
// --- Define Error Response Interface (Based on example) ---
...
export async function handleDeleteFile(
    ...
    try {
        ...
        if (err.response?.data?.error) {
            const apiError = err.response.data.error as DeleteErrorResponse['error'];
