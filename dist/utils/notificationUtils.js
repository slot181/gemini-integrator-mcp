...
import { ONEBOT_HTTP_URL, ONEBOT_ACCESS_TOKEN, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, ONEBOT_MESSAGE_TYPE, ONEBOT_TARGET_ID, REQUEST_TIMEOUT } from '../config.js';
/**
 * Sends a notification message using OneBot v11 HTTP API based on configuration.
 */
export async function sendOneBotNotification(message) {
 ...
 */
export function getConfiguredNotifiers() {
    const configured = [];
    if (!!(ONEBOT_HTTP_URL && ONEBOT_MESSAGE_TYPE && ONEBOT_TARGET_ID)) {
